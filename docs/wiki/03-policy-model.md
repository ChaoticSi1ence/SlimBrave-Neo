# 03. 策略模型（Managed Policies）

## 背景：Chromium/Brave 托管策略

SlimBrave Neo 的核心行为是写入 Chromium “managed enterprise policies”，Brave 启动时会读取这些策略并将其视为托管配置，从而在 UI 中锁定/强制某些选项。
...
- **Feature**：一个可勾选的策略键值对（policyKey → value）
- **DnsMode / DnsTemplates**：DNS over HTTPS 的模式选择与模板 URL

Python 端使用 `CATEGORIES` 列表描述 Feature 集合，并在 TUI 渲染为可勾选行：
...
## 原子写入（避免部分写入与 symlink 风险）

Linux/macOS 的策略落盘使用 `_atomic_write()`：

- 在同目录创建 `mkstemp` 临时文件（O_EXCL），避免被 symlink 劫持
