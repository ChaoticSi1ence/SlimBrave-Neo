...
如果允许任意 `--policy-file`，配合 `--reset` 可能导致在 permissive sudoers 下删除任意文件。
...
- 只写 managed policies；最终的生效依赖 Brave 自身按平台规范读取这些策略
